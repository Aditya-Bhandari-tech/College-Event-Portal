            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 md:gap-4 flex-1 sm:flex-none justify-end">
                ...
                <div className="relative">
                  ...
                  {showProfileMenu && (
                    <div className="absolute right-0 mt-2 w-56 md:w-64 bg-white rounded-2xl shadow-2xl border border-slate-200 py-2 animate-fadeIn" role="menu">
                      <div className="px-4 py-3 border-b border-slate-100">
                        <p className="font-semibold text-slate-900">{user.name}</p>
                        <p className="text-sm text-slate-600 truncate">{user.email}</p>
                      </div>
                      <button className="w-full px-4 py-2 text-left hover:bg-slate-50 flex items-center gap-3 text-slate-700" role="menuitem">
                        <User size={16} />
                      ...
                      <button onClick={handleLogout} className="w-full px-4 py-2 text-left hover:bg-slate-50 flex items-center gap-3 text-red-600 border-t border-slate-100 mt-1" role="menuitem">
                        <LogOut size={16} />
          ...
          {activeRoute === 'Dashboard' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
              ...
              <div className="space-y-4 md:space-y-6">
                {/* User Profile Card */}
                <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-4 md:p-6 text-white shadow-lg shadow-blue-500/20">
                  <div className="flex flex-col items-center text-center">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center font-bold text-xl md:text-2xl mb-3 md:mb-4 shadow-lg">
                      {user.avatar}
                    </div>
                    <h3 className="font-bold text-lg md:text-xl mb-1">{user.name}</h3>
                    <span className="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs md:text-sm font-medium mb-3 md:mb-4 capitalize">
                      {user.role}
                    </span>
                    <div className="w-full space-y-2 md:space-y-3 text-sm">
                      <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-lg p-2.5 md:p-3">
                        <Users size={14} />
